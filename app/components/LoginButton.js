...
export default function LoginButton() {
	...
	if (session) {
		return (
			<div className="flex items-center gap-3 p-4 bg-white rounded-lg shadow-md">
				...
				<p className="text-gray-700 font-medium">Signed in as {session.user.email}</p>
				<button
					onClick={() => signOut()}
					className="ml-4 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow transition-colors duration-200 font-semibold"
				>
					Sign out
				</button>
			</div>
	...
	return (
		<div className="flex flex-col items-start gap-2 p-4 bg-white rounded-lg shadow-md">
			<p className="text-gray-700 font-medium">Not signed in</p>
			<button
				onClick={() => signIn("google")}
				className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-full shadow hover:shadow-lg transition-all duration-200 font-semibold text-gray-700 hover:bg-gray-50"
			>
